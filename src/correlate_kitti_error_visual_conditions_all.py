...
def process_sequence(sequence):
    ...
    print(f"\n=== {name} ===")
    print(f"Matched samples: {len(merged_visual)}")
    ...
    if "fast_texture_score" in merged_visual.columns:
        cols.insert(3, "fast_texture_score")

    print(merged_visual[cols].corr()["aligned_position_error_m"].sort_values())
...
def main():
    ...
    if not all_rows:
        print("No KITTI sequences processed.")
        return

    combined = pd.concat(all_rows, ignore_index=True)
    combined_out = OUT_DIR / f"all_kitti_sequences_{METHOD}.csv"
    combined.to_csv(combined_out, index=False)

    print(f"\nSaved combined KITTI file: {combined_out}")
