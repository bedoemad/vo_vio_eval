...
OUT_DIR = Path("results/error_visual_correlation/binned/plots")
...
for metric in df["visual_metric"].unique():
    metric_df = df[df["visual_metric"] == metric]

    for sequence in metric_df["sequence"].unique():
        seq_df = metric_df[metric_df["sequence"] == sequence].copy()
